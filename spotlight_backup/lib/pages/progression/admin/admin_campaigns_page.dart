...
class _AdminCampaignsPageState extends State<AdminCampaignsPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      final rows = await _client
          .from('campaigns')
          .select('id, title, summary, status, starts_at, ends_at, primary_audience, priority, visibility, updated_at')
  ...
  Future<void> _create() async {
    ...
    try {
      ...
      await _client.functions.invoke('admin_campaign_upsert', body: {'campaign': created, 'reason': 'Admin create'});
      await _load();
    } catch (e) {
  ...
  Future<void> _edit(Map<String, dynamic> campaign) async {
    ...
    try {
      setState(() => _mutating = true);
      await _client.functions.invoke('admin_campaign_upsert', body: {'campaign': edited, 'reason': 'Admin edit'});
      await _load();
    } catch (e) {
  ...
  Future<void> _delete(String campaignId, String title) async {
        ...
        return SafeArea(
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                Text('Delete campaign?', style: theme.textTheme.titleLarge?.bold),
    ...
    try {
      setState(() => _mutating = true);
      await _client.functions.invoke('admin_campaign_delete', body: {'campaign_id': campaignId, 'reason': 'Admin delete'});
      await _load();
    } catch (e) {
...
class _CampaignEditorSheetState extends State<_CampaignEditorSheet> {
  ...
  void initState() {
    ...
    _summary = TextEditingController(text: (initial?['summary'] ?? '').toString());
    _description = TextEditingController(text: (initial?['description'] ?? '').toString());
    _priority = TextEditingController(text: ((initial?['priority'] ?? 10) as Object).toString());
    _primaryAudience = TextEditingController(text: ((initial?['primary_audience'] ?? 'Creators') as Object).toString());
    final actions = initial?['primary_actions'];
    final actionsText = actions is List ? actions.map((e) => e.toString()).join(', ') : (actions ?? 'post').toString();
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      ...
      child: Column(
        ...
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          Text(_editingId == null ? 'Create campaign' : 'Edit campaign', style: theme.textTheme.titleLarge?.bold),
          const SizedBox(height: AppSpacing.md),
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: AppSpacing.sm),
                  TextField(controller: _description, minLines: 3, maxLines: 6, decoration: const InputDecoration(labelText: 'Description')),
                  const SizedBox(height: AppSpacing.sm),
                  ...
                  const SizedBox(height: AppSpacing.sm),
                  TextField(controller: _primaryAudience, decoration: const InputDecoration(labelText: 'Primary audience')),
                  const SizedBox(height: AppSpacing.sm),
                  TextField(controller: _primaryActions, decoration: const InputDecoration(labelText: 'Primary actions (comma separated)')),
                  const SizedBox(height: AppSpacing.sm),
                  TextField(controller: _priority, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Priority (higher = more featured)')),
                ],
              ),
            ),
          ),
          FilledButton(
            onPressed: () {
              ...
              context.pop({
                ...
                'primary_actions': _primaryActions.text.split(',').map((e) => e.trim()).where((e) => e.isNotEmpty).toList(),
