...
class _SecurityGateScreenState extends State<SecurityGateScreen> {
          ...
          children: [
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  const Text('ACTIVE JWT AUTH CLAIMS', style: TextStyle(color: Colors.grey, fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1)),
                  const SizedBox(height: 16),
                  _buildClaimRow('ISSUER NODE', 'SUPABASE_AUTH_ROUTER'),
                  _buildClaimRow('SUBJECT UUID', currentUser?.id ?? 'NULL_POINTER'),
                  _buildClaimRow('ROLE CLAIM', 'AUTHENTICATED_OPERATOR'),
                  _buildClaimRow('CRYPTO METRIC', 'SHA-256 / RLS_ACTIVE', displayColor: const Color(0xFF39FF14)),
                ],
              ),
            ),
            ...
            FilledButton.icon(
              ...
              label: _isReauthorizing
                  ...
                  : const Text('REVERIFY TOKEN CLAIMS', style: TextStyle(fontWeight: FontWeight.bold, letterSpacing: 1, fontSize: 12)),
            ),
            ...
            Expanded(
              child: StreamBuilder<List<Map<String, dynamic>>>(
                stream: _supabase.from('security_audit_logs').stream(primaryKey: ['id']).order('timestamp'),
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Container(
                              ...
                              child: Text(
                                log['status'] ?? 'ENFORCED',
                                style: const TextStyle(color: Color(0xFF39FF14), fontSize: 9, fontWeight: FontWeight.bold, letterSpacing: 0.5),
                              ),
                            ),
                          ],
                        ),
  ...
  Widget _buildClaimRow(String label, String value, {Color displayColor = Colors.white}) {
    return Padding(
      padding: const EdgeInsets.only(bottom: 10.0),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          Text(label, style: TextStyle(color: Colors.white.withValues(alpha: 0.3), fontSize: 11, letterSpacing: 0.5)),
          Expanded(
            child: Text(
              value,
              textAlign: TextAlign.end,
              style: TextStyle(color: displayColor, fontSize: 11, fontFamily: 'monospace'),
