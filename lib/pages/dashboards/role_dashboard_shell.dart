...
/// Tab navigation for children inside [RoleDashboardShell].
class RoleShellNav extends InheritedWidget {
  const RoleShellNav({
    super.key,
  ...
  final void Function(String label) goToLabel;

  static RoleShellNav? maybeOf(BuildContext context) =>
      context.dependOnInheritedWidgetOfExactType<RoleShellNav>();

  static RoleShellNav of(BuildContext context) {
    final nav = maybeOf(context);
    assert(nav != null, 'RoleShellNav missing — only use under RoleDashboardShell');
    return nav!;
  }

  @override
  bool updateShouldNotify(RoleShellNav oldWidget) => false;
}
...
class _RoleDashboardShellState extends State<RoleDashboardShell> {
  ...
  Widget build(BuildContext context) {
    ...
    void goToLabel(String label) {
      ...
      if (i >= 0) _onIndexChanged(i);
    }

    return RoleShellNav(
      goToIndex: _onIndexChanged,
      goToLabel: goToLabel,
      child: Scaffold(
