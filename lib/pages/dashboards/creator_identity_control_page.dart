...
class _CreatorIdentityControlPageState
  ...
  void _save() {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text(IdentityFixtures.disclosure)));
  }
  ...
  Widget _buildControls() {
    return _ControlsColumn(
      ...
      onDirectContact: (value) =>
          _setUnsaved(() => _allowDirectContact = value),
      onShowLocation: (value) => _setUnsaved(() => _showLocation = value),
      onAvailability: (value) => _setUnsaved(() => _availability = value),
      onNotifyCollaborations: (value) =>
          _setUnsaved(() => _notifyCollaborations = value),
...
class _ControlsColumn extends StatelessWidget {
  const _ControlsColumn({
    ...
    required this.notifyCollaborations,
    required this.notifyCommunity,
    required this.onPublicProfile,
    required this.onDirectContact,
    required this.onShowLocation,
    required this.onAvailability,
    required this.onNotifyCollaborations,
    required this.onNotifyCommunity,
    ...
    return Column(
      children: [
          ...
          child: Column(
            children: [
              ...
              _DisabledAction(
                ...
                reason: 'Review workflow unavailable in this build.',
                onTap: () => onExplain(
                  'Review workflow unavailable in this build. Not an identity-verification result.',
                ),
              ),
            ],
          ),
        ...
        _NumberedSection(
          ...
          child: Column(
            children: [
              ...IdentityFixtures.accounts.map(
                (account) => _AccountRow(
                  account: account,
                  onTap: () => onExplain(account.detail),
                ),
              ),
            ],
          ),
        ...
        _NumberedSection(
          index: '04',
          title: 'Visibility & contact rules',
          child: Column(
            children: [
              _ToggleRow(
                label: 'Show public preview',
                value: publicProfile,
                onChanged: onPublicProfile,
              ),
              _ToggleRow(
                label: 'Allow direct contact requests',
                value: allowDirectContact,
                onChanged: onDirectContact,
              ),
              _ToggleRow(
                label: 'Show location line',
                value: showLocation,
                onChanged: onShowLocation,
              ),
            ],
          ),
        ),
              ...
              Wrap(
                ...
                children: [
                  for (final option in const [
                    'Preview only',
                    'Limited inquiry window',
                    'Not bookable here',
                  ])
                    ChoiceChip(
                      label: Text(option),
                      selected: availability == option,
                      onSelected: (_) => onAvailability(option),
                    ),
                ],
              ),
              const SizedBox(height: 10),
              _DisabledAction(
                label: 'Connect booking system',
                reason:
                    'Booking-system connection is unavailable in this build.',
                onTap: () => onExplain(
                  'Booking-system connection is unavailable in this build.',
                ),
              ),
        ...
        _NumberedSection(
          ...
          child: Column(
            children: [
              _ToggleRow(
                label: 'Collaboration briefs',
                value: notifyCollaborations,
                onChanged: onNotifyCollaborations,
              ),
              _ToggleRow(
                label: 'Community activity',
                value: notifyCommunity,
                onChanged: onNotifyCommunity,
              ),
            ],
          ),
        ),
          ...
          child: Column(
            children: [
              ...
              _DisabledAction(
                ...
                reason: 'Privacy export is unavailable in this build.',
                onTap: () =>
                    onExplain('Privacy export is unavailable in this build.'),
              ),
            ],
          ),
        ),
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          _SoftChip(label: item.state.label),
        ],
      ),
    );
  }
...
class _AccountRow extends StatelessWidget {
  ...
  final IdentityAccountFixture account;
