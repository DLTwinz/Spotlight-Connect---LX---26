...
class ProgressionService extends ChangeNotifier {
  ...
  Future<MissionModel?> loadMissionById(String missionId) async {
    final row = await _client
        .from('missions')
        .select('*')
        .eq('id', missionId)
    ...
    if (row == null) return null;
    return MissionModel.fromJson(Map<String, dynamic>.from(row));
  }
