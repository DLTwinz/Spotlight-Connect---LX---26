...
class MessagingPipelineScreen extends StatefulWidget {
  ...
  @override
  State<MessagingPipelineScreen> createState() => _MessagingPipelineScreenState();
}
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('TRANSMISSION SUCCESSFUL: Twilio pipeline telemetry updated.'),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('TRANSMISSION FAILED: Security clearing or RLS violation.'),
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Expanded(child: _buildStatWidget('PIPELINE DELIVERABILITY', '100%', const Color(0xFF39FF14))),
                const SizedBox(width: 16),
                Expanded(child: _buildStatWidget('LATENCY RATE', '24ms', const Color(0xFFD4AF37))),
              ],
            ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  TextFormField(
                    ...
                    decoration: _buildInputDecoration('Target Phone Matrix (e.g. +15015550199)', Icons.phone_android),
                    validator: (v) => (v == null || v.isEmpty) ? 'Target node required.' : null,
                  ),
                  const SizedBox(height: 12),
                  TextFormField(
                    ...
                    style: const TextStyle(color: Colors.white, fontSize: 14),
                    maxLines: 2,
                    decoration: _buildInputDecoration('Enter message packet transmission payload...', Icons.code),
                    validator: (v) => (v == null || v.isEmpty) ? 'Payload definition required.' : null,
                  ),
                  ...
                  FilledButton(
                    ...
                    child: _isSending 
                        ...
                        : const Text('DISPATCH SIGNAL', style: TextStyle(fontWeight: FontWeight.bold, letterSpacing: 1)),
                  ),
                ],
              ),
            ),
            ...
            Expanded(
              child: StreamBuilder<List<Map<String, dynamic>>>(
                stream: _supabase.from('messaging_logs').stream(primaryKey: ['id']).order('created_at'),
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    return const Center(child: CircularProgressIndicator(color: Color(0xFF39FF14)));
                  }
                  if (!snapshot.hasData || snapshot.data!.isEmpty) {
                    return const Center(child: Text('No packet traffic running in current sector.', style: TextStyle(color: Colors.grey, fontSize: 12)));
                  }
                          ...
                          children: [
                            Row(
                              ...
                              children: [
                                Text(
                                  log['recipient_phone'] ?? '',
                                  style: const TextStyle(color: Color(0xFF39FF14), fontSize: 13, fontWeight: FontWeight.bold),
                                ),
                                Container(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(status, style: TextStyle(color: displayColor, fontSize: 22, fontWeight: FontWeight.bold)),
        ],
      ),
    );
  }
