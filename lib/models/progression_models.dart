...
class ProofEventView {
  ...
  factory ProofEventView.fromJson(Map<String, dynamic> json) {
    ...
    final kind = ProofEventKind.values.firstWhere(
      (e) => e.toString().split('.').last == kindStr,
    ...
    return ProofEventView(
      ...
      prestigeDelta:
          int.tryParse(
            (json['prestige_delta'] ?? json['prestigeDelta'] ?? '0').toString(),
          ) ??
          0,
    );
  }
...
class UserProgressionModel {
  ...
  factory UserProgressionModel.fromJson(Map<String, dynamic> json) =>
      UserProgressionModel(
        ...
        prestigeTotal:
            int.tryParse((json['prestige_total'] ?? '0').toString()) ?? 0,
        prestigeThisSeason:
            int.tryParse((json['prestige_this_season'] ?? '0').toString()) ?? 0,
        momentumScore:
            int.tryParse((json['momentum_score'] ?? '0').toString()) ?? 0,
        missionsCompleted:
            int.tryParse((json['missions_completed'] ?? '0').toString()) ?? 0,
        milestonesCompleted:
            int.tryParse((json['milestones_completed'] ?? '0').toString()) ?? 0,
        campaignsParticipated:
            int.tryParse((json['campaigns_participated'] ?? '0').toString()) ??
            0,
        nextTierPrestigeRequired: json['next_tier_prestige_required'] == null
            ? null
            : int.tryParse(json['next_tier_prestige_required'].toString()),
      );
}
...
class MissionModel {
  ...
  factory MissionModel.fromJson(Map<String, dynamic> json) => MissionModel(
    ...
    prestigeReward:
        int.tryParse((json['prestige_reward'] ?? '0').toString()) ?? 0,
